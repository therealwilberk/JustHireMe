...
<!-- The sequence of work. Written at a task level — specific enough to act on,
     ...
     Tag every task AFK or HITL — see agent-primer.md Section 7. -->

### Vertical slice check _(Feature phases only — skip for Infra/Chore)_
Before listing tasks, confirm this feature crosses all necessary layers:
- [ ] Data layer (schema, migration, model)
- [ ] Logic / service layer
- [ ] Interface layer (API endpoint, CLI output, or UI surface)
- [ ] Produces something observable/testable at end of implementation

> If any layer is missing, this is likely a horizontal slice. Restructure before proceeding.

### Tasks

- [ ] `[AFK]` **Task 1:** _[e.g. "Create database schema for users table with migration"]_
- [ ] `[AFK]` **Task 2:** _[e.g. "Implement password hashing utility — use bcrypt, no custom crypto"]_
- [ ] `[AFK]` **Task 3:** _[e.g. "Build POST /auth/login endpoint — validate input, query db, return JWT"]_
- [ ] `[AFK]` **Task 4:** _[e.g. "Add JWT validation middleware for protected routes"]_
- [ ] `[AFK]` **Task 5:** _[e.g. "Write unit tests for hashing utility and login handler"]_
- [ ] `[HITL]` **Task 6:** _[e.g. "Review login UI flow — confirm error states match design intent"]_

**Blocking relationships:**
- Task 3 blocked by Task 1, Task 2
- Task 5 blocked by Task 3

> Tasks are sequential unless blocking relationships say otherwise.
> If a task reveals unexpected complexity, stop and update this plan before continuing.
> Each task should complete within one focused session — if it can't, split it.
